...
JNIEXPORT jboolean JNICALL Java_bo_htakey_rimic_audio_javacpp_Logmon_start_1ticks(JNIEnv* env, jclass cls, jint arg0, jint arg1) {
    jboolean rarg = 0;
    jthrowable exc = NULL;
    try {
        unsigned char rvalue = start_ticks(arg0, arg1);
        rarg = (jboolean)rvalue;
    } catch (...) {
...
JNIEXPORT jboolean JNICALL Java_bo_htakey_rimic_audio_javacpp_Logmon_stop_1ticks(JNIEnv* env, jclass cls) {
    jboolean rarg = 0;
    jthrowable exc = NULL;
    try {
        unsigned char rvalue = stop_ticks();
        rarg = (jboolean)rvalue;
    } catch (...) {
        exc = JavaCPP_handleException(env, 14);
    }
